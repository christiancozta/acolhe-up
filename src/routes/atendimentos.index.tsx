...
export const Route = createFileRoute("/atendimentos/")({
  head: () => ({
    meta: [
      { title: "Atendimentos | Acolhe Up" },
      {
        name: "description",
        content:
          "Consulte, busque e filtre os atendimentos registrados durante a ação de acolhimento.",
      },
      { property: "og:title", content: "Atendimentos · Acolhimento" },
      {
        property: "og:description",
        content: "Consulte, busque e filtre os atendimentos registrados.",
      },
    ],
  }),
...
function Atendimentos() {
  ...
  return (
    <Page className="pb-14">
        ...
        {CATEGORIAS.map((c) => {
          ...
          return (
            <button
              ...
              className={cn(
                "grid min-h-20 border-r border-line px-3 py-3 text-left transition-colors last:border-r-0 md:min-h-24 md:px-6",
                active ? "bg-accent text-accent-foreground" : "hover:bg-accent-soft",
              )}
            >
              <span
                className={cn(
                  "num-display text-3xl leading-none md:text-4xl",
                  !active && "text-foreground",
                )}
              >
                {String(contagem(c.id)).padStart(2, "0")}
              </span>
              <span className={cn("label-xs self-end", active ? "text-surface/70" : "text-muted")}>
                {c.label}
              </span>
            </button>
          );
      ...
      <section className="grid border-b border-line py-5 md:grid-cols-[minmax(0,1.5fr)_minmax(220px,0.5fr)] md:items-stretch">
        <Input
          value={busca}
          ...
          {lista.map((a, index) => (
            <li key={a.id}>
              <Link
                to="/atendimentos/$id"
                params={{ id: a.id }}
                className="group grid border-b border-line py-6 transition-colors last:border-b-0 hover:text-accent md:grid-cols-[44px_minmax(0,1.35fr)_minmax(180px,0.65fr)_auto] md:items-center md:gap-6"
              >
                <span className="label-xs hidden text-subtle md:block">
                  {String(index + 1).padStart(2, "0")}
                </span>
                <div className="min-w-0">
                  <div className="flex items-center justify-between gap-4 md:block">
                    <h2 className="truncate text-[1.05rem] font-medium tracking-[-0.015em] md:text-[1.18rem]">
                      {a.nome}
                    </h2>
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-foreground/80 md:hidden">
                ...
                <div className="mt-4 flex items-center gap-4 md:mt-0 md:w-[9.5rem] md:flex-col md:items-end md:gap-2.5">
                  ...
                  <span className="label-xs ml-auto text-subtle md:ml-0">
      ...
      <div className="flex justify-between gap-6 border-b border-line py-6">
        <p className="text-[0.7rem] leading-relaxed text-subtle">
          Dados armazenados somente neste navegador.
        </p>
        <Link to="/novo" className="label-xs text-accent transition-colors hover:text-foreground">
          + Novo atendimento
        </Link>
      </div>
    </Page>
...
function Vazio({ categoria, busca }: { categoria: string; busca: string }) {
  return (
    <div className="border-b border-foreground py-12">
      <div>
        <p className="text-lg font-medium tracking-[-0.02em]">
          Nenhum registro em {categoria.toLowerCase()}
        </p>
        <p className="mt-3 max-w-md text-sm leading-relaxed text-muted">
          {busca
            ? "A busca atual não encontrou correspondências nesta categoria."
            : "Os atendimentos aparecerão aqui conforme avançarem no fluxo."}
        </p>
      </div>
      <div className="mt-7">
        <Link to="/novo" className="label-xs text-accent transition-colors hover:text-foreground">
